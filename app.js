...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadBookingsFromStorage() {
        return fetch(getApiUrl('/api/bookings'))
            ...
            .catch(err => {
                console.warn("Failed to load from database, trying fallback:", err);
                const fallbackData = localStorage.getItem('sjp_bookings_fallback');
                if (fallbackData) {
                    try {
                        bookingsList = JSON.parse(fallbackData) || [];
                        showToast("เชื่อมต่อฐานข้อมูลหลักไม่ได้ ใช้งานข้อมูลชั่วคราวในเครื่อง", "info");
                    } catch (e) {
                        bookingsList = [];
    ...
    function saveCurrentBooking() {
        ...
        .catch(err => {
            ...
            if (isUpdate) {
                bookingsList[existingIndex] = bookingData;
                showToast(`อัปเดตข้อมูลรหัส ${bookingId} ในเครื่องชั่วคราว (ออฟไลน์)`, 'info');
            } else {
                bookingsList.push(bookingData);
                showToast(`บันทึกคิวงานรหัส ${bookingId} ในเครื่องชั่วคราว (ออฟไลน์)`, 'info');
            }
            try {
                localStorage.setItem('sjp_bookings_fallback', JSON.stringify(bookingsList));
            } catch (e) {}
    ...
    function deleteBooking(bookingId) {
        ...
        .catch(err => {
            ...
            if (index > -1) {
                ...
                showToast(`ลบข้อมูลรหัส ${bookingId} จากเครื่องชั่วคราว (ออฟไลน์)`, 'info');
            } else {
    ...
    function updateOnlineStatus() {
        ...
        if (navigator.onLine) {
            ...
            statusTextEl.innerText = 'ออนไลน์';
        } else {
            statusEl.className = 'connection-status offline';
            statusTextEl.innerText = 'ออฟไลน์';
        }
    }
        ...
        btnClearAll.addEventListener('click', () => {
            if (confirm("คุณต้องการล้างข้อมูลคิวงานทั้งหมดหรือไม่?")) {
                ...
                .catch(err => {
                    ...
                    updateDashboardStats();
                    renderBookingsTable();
                    showToast("ล้างข้อมูลคิวงานในเครื่องเรียบร้อยแล้ว (ออฟไลน์)", "info");
                });
            }
        });
    }
